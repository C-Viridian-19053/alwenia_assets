...
function spawnTempoQuartzPattern(mNumbSpawn)
    ...
    elseif mNumbSpawn == 1 then
        if GLOBAL_TEMPO > 90 then
            spMarch31osWhirlwindRev(_side, 0, 0, 1, 1, 2, 1.5, getRandomDir(), 1, 1, 1, false); t_wait(customizeTempoPatternDelay(1));
        else
            spMarch31osWhirlwindRev(_side, 12, 0, 0, 1, 1, .5, getRandomDir(), 0, 0, 0, false); t_wait(customizeTempoPatternDelay(1));
        end
    ...
    elseif mNumbSpawn == 3 then
        ...
        t_wait(customizeTempoPatternDelay(.75 * _del));
        ...
        t_wait(customizeTempoPatternDelay(.5 * _del));
        if getProtocolSides() > 5 then
            cGrowWall(_side, getProtocolSides() > 5 and math.floor(getProtocolSides() / 4) or 0, customizeTempoPatternThickness(.5 * _del))
        end
        t_wait(customizeTempoPatternDelay(.25 * _del));
        cGrowWall(_side, math.floor(getProtocolSides() / 2) - 1, customizeTempoPatternThickness(0.25 * _del))
        t_wait(customizeTempoPatternDelay(4));
