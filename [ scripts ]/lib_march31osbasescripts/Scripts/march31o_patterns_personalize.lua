--[[
    void pMarch31osExRandomBarrage(_side, _thickness, _iter, _distMin, _distMax, _isRepeat, _isDelayDistance, _delMult, _sizeMult)
    void pMarch31osExRandomBarrage(_side, _thickness, _iter) --, -getHalfSides("floor"), getHalfSides("floor"), true, true, 1, 1
    void pMarch31osExAlternatingBarrage(_side, _thickness, _iter, _isSpiral, _hasContainedEnd, _gapLength, _barrageDir, _repeatBarrageAmount, _repeatBarrageDelay, _isRepeatBarrageDelaySpd, _delMult, _sizeMult, _direction)
    void pMarch31osExAlternatingBarrage(_side, _thickness, _iter) --, false, false, 0, getRandomDir(), 0, 2, false, 1, 1, getRandomDir()
    void pMarch31osExVortaBarrage(_side, _thickness, _iter, _dirType, _revFreq, _dirPoints, _modeType, _free, _delMult, _sizeMult, _direction)
    void pMarch31osExVortaBarrage(_side, _thickness, _iter) --, 0, 1, {math.floor(_iter / 2)}, 0, 0, 1, 1, getRandomDir()
    void pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, _holes, _offsetType, _offsetDistance, _mainLoopDir, _isInvertAccurate, _delMult, _sizeMult)
    void pMarch31osBarrageTypeBase(_side, _thickness, _iter) --, 1, 1, 1, 1, getRandomDir(), false, 1, 1

    void pMarch31osExTriWhirlwind(_side, _iter, _revFreq, _sizeMult, _direction)
    void pMarch31osExTriWhirlwind(_side, _iter) --, 0, 1, getRandomDir()
    void pMarch31osExQuadWhirlwind(_side, _iter, _revFreq, _sizeMult, _direction)
    void pMarch31osExQuadWhirlwind(_side, _iter) --, 0, 1, getRandomDir()
    void pMarch31osExWhirlwind(_side, _iter, _timesBeforeChangeDir, _revFreq, _extraWidth, _mirrorStep, _posSpacing, _seamless, _delMult, _sizeMult, _direction, _slopeAmountStart, _slopeAmountCycle, _slopeAmountEnd, _is_full)
    void pMarch31osExWhirlwind(_side, _iter) --, 1, 0, 1, math.floor(getProtocolSides() / 3), 1, true, 1, 1, getRandomDir(), 0, 0, 0, false

    void pMarch31osExTunnel(_side, _corridorThickOfSpeedLessThan, _corridorThickOfSpeedGEThan, _iter, _largeWalls, _delMult, _sizeMult, _direction, _dirType, _largeWallDesign, _isDistanceTight, _isLargeWallOnce, _sidedir0gap, _sidedir1gap, _sidedir0offset, _sidedir1offset, _gearTeethSizeMult, _gearTeethInc, _gearTeethStepDel, _gearTeethStepLimit, _isBeforeGearTeethBegin, _isAfterGearTeethEnd, _repeatCorridorAmount, _repeatCorridorDelay, _isRepeatCorridorDelaySpd)
    void pMarch31osExTunnel(_side, _corridorThickOfSpeedLessThan, _corridorThickOfSpeedGEThan, _iter) --, 1, 1, 1, u_rndInt(0, 1), 0, 0, true, false, 1, 1, 0, 0, 0, (to be indexed if is nil), 1, 1, false, false, 0, 0, false
]]
...
-- pMarch31osExRandomBarrage(): spawns barrages with random side, and waits humanly-possible times depending on the sides distance
...
function pMarch31osExRandomBarrage(_side, _thickness, _iter, _distMin, _distMax, _isRepeat, _isDelayDistance, _delMult, _sizeMult)
    ...
    for a = 0, _iter, 1 do
        ...
        if getBooleanNumber(_isDelayDistance) then
            if _barrageDistanceDelay < 0 then
                _barrageDistanceDelay = _barrageDistanceDelay * -1;
            end
...
-- pMarch31osExAlternatingBarrage(): spawns a series of cWallExM
...
--              _barrageDir: which dir to gap length to add
...
function pMarch31osExAlternatingBarrage(_side, _thickness, _iter, _isSpiral, _hasContainedEnd, _gapLength, _barrageDir, _repeatBarrageAmount, _repeatBarrageDelay, _isRepeatBarrageDelaySpd, _delMult, _sizeMult, _direction)
    ...
    _barrageDir = (type(_direction) == "number" and getNeg(_direction)) or (_direction == 0 and getRandomDir()) or _direction; -- barrage dir bug
    ...
    TARGET_PATTERN_SIDE = p_getRebootingSideBool() and _curSide or -256;
    local _barrageOffset, _barrageLoopDir, _barrageDirStr = 0, _direction, _barrageDir;

    for a = 0, _iter, 1 do
        p_patternEffectCycle();

        for repeatAmount = 0, _repeatBarrageAmount, 1 do
            cWallExM(_curSide + _barrageOffset, (math.floor(getProtocolSides() / 2) - _gapLength) + (_barrageDirStr * 0.5 - 0.5), 2, p_getPatternThickness() * _sizeMult);
        ...
        if (not _isSpiral) then
            ...
            _barrageDirStr = _barrageDirStr * -1;
        end
    ...
    if getBooleanNumber(_hasContainedEnd) then
        for repeatAmount = 0, _repeatBarrageAmount, 1 do
            ...
            elseif getProtocolSides() > 5  then cDoubleHoledBarrage(_curSide + _barrageOffset + (math.floor(_gapLength / 2) * 2), 0, 0, p_getPatternThickness() * _sizeMult);
            end
            if repeatAmount < _repeatBarrageAmount then t_applyPatDel(customizePatternDelay(_repeatBarrageDelay * _sizeMult, _isRepeatBarrageDelaySpd)); end
        end
    end
...
function pMarch31osExVortaBarrage(_side, _thickness, _iter, _dirType, _revFreq, _dirPoints, _vortaBarType, _free, _delMult, _sizeMult, _direction)
    -- optional args
...
function pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, _holes, _offsetType, _offsetDistance, _mainLoopDir, _isInvertAccurate, _delMult, _sizeMult)
    ...
    for a = 0, _iter, 1 do
        ...
        if a % 2 == 1 and (getProtocolSides() % 2 == 1) and _holes == 1 and _offsetType == 2 and getBooleanNumber(_isInvertAccurate) then cWallEx(_curSide + 1 + getHalfSides(), getProtocolSides() - (2 + _gap), p_getPatternThickness());
        else
...
function pMarch31osBarrageSpiral(_side, _thickness, _iter, _gap, _distance, _isDelayDistance, _delMult, _sizeMult, _direction)
    pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, 1, 0, anythingButNil(_distance, 1), _direction, false, (getBooleanNumber(_isDelayDistance) and (anythingButNil(_distance, 1) * 0.333 + 0.667) or 1) * _delMult, _sizeMult)
end
...
function pMarch31osBarrageReversals(_side, _thickness, _iter, _gap, _isInvertAccurate, _delMult, _sizeMult, _direction)
    pMarch31osBarrageTypeBase(_side, _thickness, _iter, _gap, 1, 2, 1, _direction, _isInvertAccurate, _delMult * 2, _sizeMult)
end

-- [ Spirals ] --

-- pMarch31osExTriWhirlwind(): spawns a spiral of cMirrorWallEx where you need to change direction
-- _revFreq: amount of times of direction changes
function pMarch31osExTriWhirlwind(_side, _iter, _revFreq, _sizeMult, _direction)
    _iter = anythingButNil(_iter, u_rndInt(4, 7)); _sizeMult = anythingButNil(_sizeMult, 1);
...
-- pMarch31osExQuadWhirlwind(): spawns a spiral of cMirrorWallEx where you need to change direction
-- _revFreq: amount of times of direction changes
function pMarch31osExQuadWhirlwind(_side, _iter, _revFreq, _sizeMult, _direction)
    _iter = anythingButNil(_iter, u_rndInt(4, 7)); _sizeMult = anythingButNil(_sizeMult, 1);
    _revFreq = closeValue((type(_revFreq) == "number" and _revFreq or 0), 0, 999, "min");
    _direction = (type(_direction) == "number" and getNeg(_direction)) or (_direction == 0 and -1) or getRandomDir();
    ...
    if getProtocolSides() == 4 then
        if _revFreq > 0 then
            ...
            for a = 0, _iter + 1, 1 do
                p_patternEffectCycle();
                cWall(_curSide + _spiralPosistionOffset, customizePatternThickness((a == _iter + 1 and 6 or 9) * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
                _spiralPosistionOffset = _spiralPosistionOffset + _direction;
                t_applyPatDel(customizePatternDelay(4 * currentSizeOverride * _sizeMult, p_getDelayPatternBool()));
            end
...
-- pMarch31osExWhirlwind(): spawns a spiral of cMirrorWallEx where you need to change direction
...
function pMarch31osExWhirlwind(_side, _iter, _timesBeforeChangeDir, _revFreq, _extraWidth, _mirrorStep, _posSpacing, _seamless, _delMult, _sizeMult, _direction, _slopeAmountStart, _slopeAmountCycle, _slopeAmountEnd, _is_full)
    ...
    elseif getProtocolSides() == 4 and ((_slopeAmountStart > 0 and _slopeAmountCycle > 0 and _slopeAmountEnd > 0) and _is_full) then
        ...
        if (_slopeAmountStart > 0) then
            for fa = 0, _slopeAmountStart, 1 do
                for ia = 0, _slopeAmountStart - fa, 1 do
                    if ia > 0 then cMirrorWallEx(_curSide - (_direction * _posSpacing * (ia - 1)), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + 1) * _sizeMult, p_getDelayPatternBool())); end
                end
                cMirrorWallEx(_curSide + (_direction * _posSpacing * fa) - (_slopeAmountStart * _direction * _posSpacing), _mirrorStep, _extraWidth, customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
        ...
        for rev = 0, _revFreq, 1 do
            local _halt = _revFreq > 0 and rev == _revFreq and 1 or 0
            for a = 0, _iter - _halt, 1 do
                p_patternEffectCycle();
                cMirrorWallEx(_curSide + _spiralPosistionOffset, _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + (a == _iter - _halt and (_slopeAmountCycle > 0) and rev < _revFreq and 1 or 0)) * _sizeMult, p_getDelayPatternBool()));
                if a < _iter - _halt then
                    t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                    _spiralPosistionOffset = _spiralPosistionOffset + (_direction * _posSpacing);
                end
                if a == _iter - _halt and rev == _revFreq then _spiralPosistionOffset = _spiralPosistionOffset + (_direction * _posSpacing); end
            end
            if rev < _revFreq then
                if (_slopeAmountCycle > 0) then
                    for fb = 0, _slopeAmountCycle - 1, 1 do
                        for ib = 0, fb, 1 do cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + 1) * _sizeMult, p_getDelayPatternBool())); end
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                    end
                    if _timesBeforeChangeDir > 0 then
                        for hb = 0, _timesBeforeChangeDir - 1, 1 do
                            if hb < _timesBeforeChangeDir - 1 then
                                for ib = 0, _slopeAmountCycle, 1 do cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())); end
                            elseif hb == _timesBeforeChangeDir - 1 then
                                for ib = 0, _slopeAmountCycle - 1, 1 do cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())); end
                                cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * _slopeAmountCycle), _mirrorStep, _extraWidth, customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
                            end
                            t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                        end
                    end
                    for vb = 1, _slopeAmountCycle, 1 do
                        for eb = 0, _slopeAmountCycle - (vb + 1), 1 do
                            if eb >= 0 then
                                for lb = 0, _slopeAmountCycle - (vb + 1), 1 do cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * lb), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + 1) * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())); end
                            end
                        end
                        cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * (_slopeAmountCycle - vb)), _mirrorStep, _extraWidth, customizePatternThickness(_curSpiralThick * _sizeMult, p_getDelayPatternBool()));
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                    end
                else
                    for hb = 0, _timesBeforeChangeDir, 1 do
                        cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * _slopeAmountCycle), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + (hb < _timesBeforeChangeDir and 1 or 0)) * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                        t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
                    end
                end
        ...
        if (_slopeAmountEnd > 0) then
            for fb = 0, _slopeAmountEnd, 1 do
                for ib = 0, fb, 1 do cMirrorWallEx(_curSide + (_spiralPosistionOffset - (_direction * _posSpacing)) + (_direction * _posSpacing * ib), _mirrorStep, _extraWidth, customizePatternThickness((_curSpiralThick + (fb < _slopeAmountEnd and 1 or 0)) * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool())); end
                t_applyPatDel(customizePatternDelay(2 * currentDelayOverride * _curDelaySpeed * _delMult * _sizeMult, p_getDelayPatternBool()));
            end
        end
    end
...
-- pMarch31osExTunnel(): a base tunnel what forces you to circle around a very thick wall...but you can personalize everything.
...
function pMarch31osExTunnel(_side, _corridorThickOfSpeedLessThan, _corridorThickOfSpeedGEThan, _iter, _largeWalls, _delMult, _sizeMult, _direction, _dirType, _largeWallDesign, _isDistanceTight, _isLargeWallOnce, _sidedir0gap, _sidedir1gap, _sidedir0offset, _sidedir1offset, _gearTeethSizeMult, _gearTeethInc, _gearTeethStepDel, _gearTeethStepLimit, _isBeforeGearTeethBegin, _isAfterGearTeethEnd, _repeatCorridorAmount, _repeatCorridorDelay, _isRepeatCorridorDelaySpd)
    ...
    p_patternEffectEnd();
    t_applyPatDel(p_getEndAdditionalDelayPattern() + (getPerfectDelay(THICKNESS) * (p_getSkipEndDelayPatternBool() and 8 or 12)) + (customizePatternDelay(0.5, false) * (_largeWalls / (getProtocolSides() - 2))));
end
